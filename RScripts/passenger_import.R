...
for(i in 1:length(zips)){
  ...
  if(length(files_csv) > 0 & length(files_xls) == 0){
    ...
    stop("don't know where files are")
  }
  
  yr <- strsplit(zips[i], "/", fixed = TRUE)[[1]]
  yr <- as.numeric(substr(yr[length(yr)],1,4))
...
# 2015 - 2024
for(i in seq(length(zips) + 1, length(zips) + (max_year-2014))){
...
saveRDS(transit, "data/passenger_transit_2024.Rds")
saveRDS(int_od, "data/passenger_int_od_2024.Rds")
saveRDS(dom_od, "data/passenger_dom_od_2024.Rds")
