...
flight_od <- readRDS("data/flights_od_2024.Rds")
...
flight_od <- flight_od %>%
  ...
  summarise(airport1 = airport1[1],
            ...
            flt_2022 = max(c(flt_2022,0), na.rm = TRUE),
            flt_2023 = max(c(flt_2023,0), na.rm = TRUE),
            flt_2024 = max(c(flt_2024,0), na.rm = TRUE)
  )
...
saveRDS(flight_od, "data/flights_od_prepped_2024.Rds")
