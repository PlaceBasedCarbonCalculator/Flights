...
airports_2024 <- readRDS("data/airports_missing_2024.Rds")
...
airports_extra2 <- data.frame(airport = c("Tabarka","St Maarten","Sydney Canada","Lanseria","Izmir (Cumaovasi)","Islamabad"),
...
names(airports_extra2) <- names(airports)

names(airports_2021) <- names(airports)
names(airports_2024) <- names(airports)

airports_extra <- st_as_sf(airports_extra, crs = 4326)
airports_extra2 <- st_as_sf(airports_extra2, crs = 4326)
airports_2021 <- st_as_sf(airports_2021, crs = 4326)
airports_2024 <- st_as_sf(airports_2024, crs = 4326)

airports <- rbind(airports, airports_extra, airports_extra2, airports_2021, airports_2024)
...
flight_od$airport1_country[flight_od$airport1 == "Asmara"] <- "Eritrea"
flight_od$airport2_country[flight_od$airport2 == "Asmara"] <- "Eritrea"
...
#TODO dmo mroe check for missing airport Teran, Iran for example
saveRDS(all_od, "data/pass_flighs_od_2024.Rds")
...
stop("End of code, post run checks")
...
pass_total <- pass_od2 %>%
  ...
  summarise(total = sum(`1990`,`1991`,`1992`,`1993`,`1994`,`1995`,`1996`,`1997`,`1998`,`1999`,`2000`,`2001`,`2002`,`2003`,`2004`,`2005`,
                        `2006`,`2007`,`2008`,`2009`,`2010`,`2011`,`2012`,`2013`,`2014`,`2015`,`2016`,`2017`,`2018`,`2019`,`2020`,`2021`,`2022`,`2023`,`2024`, na.rm = TRUE))
ap_miss <- left_join(ap_miss, pass_total, by = c("airport1" = "airport2", "airport1_country" = "airport2_country"))


#saveRDS(ap_miss,"data/airports_missing_2024.Rds") # WARNING Don't overwrite 


all_missing <- all_od[is.na(all_od$`2024`) | is.na(all_od$flt_2024),]
